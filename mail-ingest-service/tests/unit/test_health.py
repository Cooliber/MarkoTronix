...
class TestHealthChecks:
    ...
    @pytest.mark.asyncio
    @patch("app.core.health.asyncio.sleep")
    async def test_check_imap_health_success(self, mock_sleep):
        """Test successful IMAP health check."""
        # Mock successful IMAP connection
        mock_sleep.return_value = None
        
        result = await check_imap_health()
        assert result.status == HealthStatus.OK
        assert "successful" in result.message
        assert isinstance(result.last_checked, datetime)
        assert isinstance(result.response_time_ms, int)
    ...
    def test_determine_overall_status_imap_critical(self):
        """Test determining overall status when IMAP is critical."""
        database = ComponentHealth(
            status=HealthStatus.OK,
            message="Connected",
            last_checked=datetime.now(),
    ...
    def test_determine_overall_status_redis_degraded(self):
        """Test determining overall status when Redis is degraded."""
        database = ComponentHealth(
            status=HealthStatus.OK,
            message="Connected",
            last_checked=datetime.now(),
