...
class HealthCheck(BaseModel):
    ...
    openai: Optional[ComponentHealth] = Field(None, description="OpenAI API health")
    storage: Optional[ComponentHealth] = Field(None, description="Storage health")
...
EXTERNAL_SERVICES = [
    ...
    # Example: {"name": "mail-ingest", "url": "http://mail-ingest:8000/health"}
]
    ...
    try:
        ...
        if OPENAI_API_KEY:
            ...
            if not models.data:
                ...
                message = "OpenAI API returned empty models list"
        else:
            # Simulate OpenAI check if API key not provided
            await asyncio.sleep(0.1)
            status = HealthStatus.DEGRADED
            message = "OpenAI API key not configured"
            
    except Exception as e:
        status = HealthStatus.DEGRADED  # OpenAI is important but not critical
...
async def check_storage_health() -> ComponentHealth:
    """Check the health of the storage directory."""
    start_time = time.time()
    status = HealthStatus.OK
    message = "Storage directory is accessible"
    ...
    try:
        ...
        if STORAGE_DIR:
            ...
            # Simulate storage check if directory not provided
            await asyncio.sleep(0.05)
            status = HealthStatus.DEGRADED
            message = "Storage directory not configured"
            
    except Exception as e:
        status = HealthStatus.CRITICAL  # Storage is critical for offer generation
        message = f"Storage directory check failed: {str(e)}"
...
def determine_overall_status(
    ...
    openai: Optional[ComponentHealth],
    storage: Optional[ComponentHealth],
    external_services: List[ServiceHealth],
    circuit_breakers: List[CircuitBreakerHealth],
) -> HealthStatus:
    """Determine the overall health status."""
    # Critical if database or storage is down
    if database.status == HealthStatus.CRITICAL or (storage and storage.status == HealthStatus.CRITICAL):
    ...
    if openai and openai.status != HealthStatus.OK:
        degraded_count += 1
    
    if storage and storage.status == HealthStatus.DEGRADED:
        degraded_count += 1
    ...
    total_components = 4 + len(external_services)  # Database, Redis, OpenAI, Storage, and external services
...
async def health_check(request: Request) -> HealthCheck:
    ...
    overall_status = determine_overall_status(
        database, redis, openai, storage, external_services, circuit_breakers
    )
    ...
    if overall_status != HealthStatus.OK:
        logger.warning(
            ...
            extra={
                ...
                "openai_status": openai.status if openai else None,
